...
export class RestaurantService {
  ...
  private validateTime(opened: string, closed: string) {
    ...
    if (!timeRegex.test(opened)) {
      return {
        valid: false,
        error: "Invalid opening time format. Use HH:MM (00:00 — 23:59)",
      };
    }
    if (!timeRegex.test(closed)) {
      return {
        valid: false,
        error: "Invalid closing time format. Use HH:MM (00:00 — 23:59)",
      };
    }
  ...
  async update(
    ...
    if (dto.timeOpened) {
      ...
      if (isNaN(h) || isNaN(m))
        throw new BadRequestException("Invalid timeOpened format");
      const d = new Date();
      d.setHours(h, m, 0, 0);
    ...
    if (dto.timeClosed) {
      ...
      if (isNaN(h) || isNaN(m))
        throw new BadRequestException("Invalid timeClosed format");
      const d = new Date();
      d.setHours(h, m, 0, 0);

...
export function OrderDiscovery() {
	...
	const { mutate: acceptOrder, isPending: isAcceptPending } = useMutation({
		...
		onError(error: AxiosError<{ message?: string | string[] }>) {
			...
			setErrorMessage(Array.isArray(message) ? message.join(', ') : message || 'Failed to accept order')
			queryClient.invalidateQueries({ queryKey: ['active-order'] })
			queryClient.invalidateQueries({ queryKey: ['order-discovery'] })
		}
	})
	...
	const { mutate: completeOrder, isPending: isCompletePending } = useMutation({
		...
		mutationFn: (orderId: string) => orderDiscoveryService.completeOrder(orderId),
		onSuccess() {
			setErrorMessage(null)
			queryClient.invalidateQueries({ queryKey: ['active-order'] })
			queryClient.invalidateQueries({ queryKey: ['order-discovery'] })
		...
		onError(error: AxiosError<{ message?: string | string[] }>) {
			const message = error.response?.data?.message
			setErrorMessage(Array.isArray(message) ? message.join(', ') : message || 'Failed to complete order')
		}
	})
	...
	const uniqueOrders = useMemo(
		() =>
			Array.from(
				new Map((data?.data ?? []).map(order => [order.idOrder, order])).values()
			),
		[data?.data]
	)
